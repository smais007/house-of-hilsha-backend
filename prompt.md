...
You are a **senior backend architect and security engineer** building a **production-ready MERN authentication system**.

Your task is to implement a **complete authentication flow** using:
...
- **Better Auth (email-password provider only)**

You MUST follow this documentation exactly:
👉 https://www.better-auth.com/docs/authentication/email-password

Do NOT invent authentication logic that Better Auth already provides.

---

## 1️⃣ Architecture Constraints (Non-Negotiable)
...
- Services handle auth logic
- Models only define schemas
- Config folder for Better Auth + environment setup

### Frontend

- React + TypeScript
- Auth state handled via Context or Zustand
- API communication via Axios or Fetch
- No auth logic duplicated on frontend

---

## 2️⃣ Authentication Features (Must Implement All)
...
- Return session or token as per Better Auth
- Secure cookie or token storage strategy
...
## 3️⃣ Folder Structure (Backend – Required)
...
## 5️⃣ Better Auth Rules (Critical)

- Use **Better Auth email-password provider**
- Do NOT hash passwords manually
- Do NOT generate custom tokens
- Use Better Auth verification & reset APIs
...
- Redirect after verification/login
- Block access to protected routes if unauthenticated
...
1. Complete backend implementation (TypeScript)
2. Frontend auth flow (React + TS)
3. Better Auth configuration
4. Environment variable list
5. Explanation of auth flow (signup → verify → login)
...
Build this as if it will be deployed to production.
